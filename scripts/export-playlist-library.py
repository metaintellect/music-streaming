#!/usr/bin/env python3
"""
Export playlist tracks into a self-contained playlist library.

Examples:
    # Dry run one playlist to Opus
    python3 scripts/export-playlist-library.py \
      --playlists-root /mnt/nas/playlists \
      --unc-root /mnt/nas \
      --output-root /mnt/nas/playlist-library-opus \
      --playlist exyu

    # Export playlist FLAC library
    python3 scripts/export-playlist-library.py \
      --playlists-root /mnt/nas/playlists \
      --unc-root /mnt/nas \
      --output-root /mnt/nas/playlist-library-flac \
      --playlist exyu \
      --format flac \
      --apply
"""
...
TEMP_FILE_PATTERNS = (
    "playlist-library-audio-*",
    "playlist-library-final-*",
    "playlist-library-meta-*",
)


class PlaylistLibraryError(Exception):
    """User-facing error."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", required=True, help="Root folder for exported playlist libraries")
    parser.add_argument(
        "--playlist",
        action="append",
    ...
    parser.add_argument(
        ...
        choices=("opus", "flac"),
        default="opus",
        help="Output audio format. Default: %(default)s",
    )
    ...
    parser.add_argument(
        ...
        help="Album artist tag for exported playlist libraries. Default: %(default)s",
    )
    parser.add_argument(
        "--genre",
        default="playlist",
        help="Genre tag for exported playlist libraries. Default: %(default)s",
    )
    parser.add_argument(
        "--prefix-track-number",
        action="store_true",
        help="Prefix exported filenames with playlist order and also write track number tags",
    )
...
def resolve_playlist_file(playlists_root: Path, requested: str) -> Path:
    ...
    raise PlaylistLibraryError(f"Playlist not found for '{requested}' in {playlists_root}")
...
def resolve_playlists(playlists_root: Path, requested: list[str] | None) -> list[Path]:
    ...
    if not playlists_root.exists():
        raise PlaylistLibraryError(f"Playlist root does not exist: {playlists_root}")

    playlists = sorted(
        path for path in playlists_root.iterdir() if path.is_file() and path.suffix.lower() in SUPPORTED_SUFFIXES
    )
    if not playlists:
        raise PlaylistLibraryError(f"No playlist files found in {playlists_root}")
    return playlists
...
def decode_playlist_path(raw_value: str, playlist_path: Path, unc_root: Path | None) -> Path:
    if raw_value.startswith("file://"):
        ...
        if parsed.scheme != "file":
            raise PlaylistLibraryError(f"Unsupported URI in {playlist_path}: {raw_value}")
        if parsed.netloc not in ("", "localhost"):
            raise PlaylistLibraryError(f"Unsupported file URI host in {playlist_path}: {raw_value}")
...
def parse_playlist(playlist_path: Path, unc_root: Path | None) -> ParsedPlaylist:
    try:
        ...
        raise PlaylistLibraryError(f"Could not read playlist {playlist_path}: {exc}") from exc
...
def output_suffix(output_format: str) -> str:
    return ".opus" if output_format == "opus" else ".flac"


def track_number_width(total_tracks: int) -> int:
    return max(2, len(str(total_tracks)))


def destination_filename(source: Path, output_format: str, *, track_number: int | None = None, total_tracks: int | None = None) -> str:
    stripped = TRACK_PREFIX_RE.sub("", source.stem, count=1)
    if track_number is not None:
        if total_tracks is None:
            raise PlaylistLibraryError("total_tracks is required when track_number is set")
        stripped = f"{track_number:0{track_number_width(total_tracks)}d} {stripped}"
    return f"{stripped}{output_suffix(output_format)}"


def parse_track_artist_title(source: Path) -> tuple[str, str]:
    stripped = TRACK_PREFIX_RE.sub("", source.stem, count=1)
    if " - " not in stripped:
        return "", stripped
...
def find_filename_collisions(
    ...
    output_format: str,
    *,
    prefix_track_number: bool,
) -> list[FilenameCollision]:
    by_name: dict[str, list[Path]] = {}
    total_tracks = len(source_paths)
    for index, source in enumerate(source_paths, start=1):
        filename = destination_filename(
            source,
            output_format,
            track_number=index if prefix_track_number else None,
            total_tracks=total_tracks if prefix_track_number else None,
        )
        by_name.setdefault(filename, []).append(source)
...
def build_export_actions(
    ...
    *,
    ...
    for index, source in enumerate(source_paths, start=1):
        track_artist, track_title = parse_track_artist_title(source)
        dest, renamed = unique_dest_path(
            dest_dir,
        ...
        actions.append(
            ExportAction(
                ...
                dest=dest,
                renamed=renamed,
                art_path=source_folder_art(source),
...
def encoder_args(output_format: str, bitrate_kbps: int, mode: str) -> list[str]:
...
def playlist_metadata_pairs(
    ...
    if track_number is not None:
        pairs.append(("TRACKNUMBER", str(track_number)))
    if track_total is not None:
        pairs.append(("TRACKTOTAL", str(track_total)))
    return pairs
...
def analyze_replaygain(source: Path, target_lufs: float) -> ReplayGainResult:
    ...
    if not matches_loudness or not matches_peak:
        raise PlaylistLibraryError(f"Could not parse loudness analysis for {source}")
...
def convert_to_temp_audio(
    ...
    *,
    ...
    for key, value in playlist_metadata_pairs(
        playlist_name,
        track_artist,
        track_title,
        album_artist,
    ...
    run_captured(
        [
            ...
            "-1",
            "-vn",
            *encoder_args(output_format, bitrate_kbps, mode),
            *metadata_args,
            str(temp_path),
        ]
    )
...
def finalize_audio(
    ...
    with NamedTemporaryFile(prefix="playlist-library-meta-", suffix=".ffmeta", dir=dest.parent, delete=False) as temp_file:
        metadata_path = Path(temp_file.name)
    metadata_path.write_text("\n".join(build_ffmetadata_lines(metadata_pairs, replaygain, art_path)) + "\n", encoding="utf-8")
    ...
    with NamedTemporaryFile(prefix="playlist-library-final-", suffix=dest.suffix, dir=dest.parent, delete=False) as temp_file:
    ...
    try:
        ...
        raise PlaylistLibraryError(f"Could not finalize {dest}: {stderr}") from exc
    finally:
    ...
    *,
    ...
    for action in actions:
        ...
        with NamedTemporaryFile(
            prefix="playlist-library-audio-",
            suffix=output_suffix(output_format),
            dir=action.dest.parent,
            delete=False,
        ) as temp_file:
            temp_audio = Path(temp_file.name)

        try:
            metadata_pairs = playlist_metadata_pairs(
                playlist_name,
                action.track_artist,
                action.track_title,
                album_artist,
                genre,
                track_number=action.track_number if prefix_track_number else None,
                track_total=track_total if prefix_track_number else None,
            )
            convert_to_temp_audio(
                action.source,
                temp_audio,
            ...
            rg_written, art_written = finalize_audio(
                temp_audio,
                action.dest,
...
def process_playlist(playlist_path: Path, args: argparse.Namespace, logger: Logger) -> int:
    ...
    playlist_name = playlist_folder_name(playlist_path)
    dest_dir = Path(args.output_root).expanduser() / playlist_name
    cleaned_temp_files: list[Path] = []
    if args.apply:
        dest_dir.mkdir(parents=True, exist_ok=True)
        cleaned_temp_files = cleanup_stale_temp_files(dest_dir)
    actions = build_export_actions(
        validation.existing_files,
        dest_dir,
        args.format,
        prefix_track_number=args.prefix_track_number,
    )
    ...
    logger.emit(f"  Output format: {args.format}")
    logger.emit(f"  Compilation album: {playlist_name}")
    logger.emit(f"  Album artist: {args.album_artist}")
    logger.emit(f"  Genre: {args.genre}")
    logger.emit(f"  Prefix track number: {'yes' if args.prefix_track_number else 'no'}")
    ...
    if not args.apply:
        logger.emit("  Dry run only. Re-run with --apply to export files.")
        return 0

    if validation.missing_paths or validation.non_file_paths:
        raise PlaylistLibraryError("Refusing to export because some playlist entries are missing or not regular files")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except PlaylistLibraryError as exc:
